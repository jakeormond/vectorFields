% relativeDirection_spikes_ShiftsHDandPos2Sink
%%
cd positionalData
...
cd physiologyData/placeFieldData
load rateMaps.mat rateMaps frameSize
cd ..


cd direction
load mrlFocus_spikes_coarse.mat
...
for tt = 1:length(trialTypes)
    ...
    maxShift.(trialTypes{tt}) = nFrames - minShift;
           
    shifts = [0; (randsample(maxShift.(trialTypes{tt}) - minShift, 1000) + minShift)];
    ...
    for c = 1:nCells
        ...
        rmInd = find(strcmp(unitName, {rateMaps(:).unit}));
        spikeSamples{c} = rateMaps(rmInd).(trialTypes{tt}).spikeSamples;
    end
    
    
    signifCells_Shift.(trialTypes{tt}) = {};
    % cellFlags = true(nCells, 1); % indicates whether cell may still be significant, or can be omitted
    %%
    ...
    parfor s = 1:length(shifts)
        ...
        for c = 1:nCells
            spikeFrameInd = round(interp1(samples, ...
                1:length(samples), double(spikeSamples{c})));
            
            spikePos = posShifted(spikeFrameInd, :);
            spikeHD = hdShifted(spikeFrameInd);
            ...
            totalDist = ctrlDistributions(spikeFrameInd, platsShifted, ...
                relDirDistsTemp);
...
%%
save mrlFocus_spikes_ShiftsHDandPos2Sink.mat angleEdges ...
    histBinCentres signifCells_Shift mrls coors directions shiftsUsed
